...
function child_theme_setup() {
	...
	add_action( 'genesis_header', 'ca_tablet_header' );
	add_action( 'genesis_before', 'ca_remove_header' );
	...
	/* add_action('genesis_entry_content', 'jp_display_custom_fields'); */
...
// tablet header
function ca_tablet_header(){
	get_template_part('partials/tablet', 'header');
}
...
function jp_display_custom_fields() {

	$date = get_field( 'date' );
	$raffle_copter_script = get_field( 'raffle_copter_script' );
	

	if ( $date || $raffle_copter_script ) {

		echo '<div class="giveaway-meta">';

			if ( $date ) {
				echo '<p><strong>Date</strong>: ' . $date . '</p>';
			}

			if ( $raffle_copter_script ) {
				echo '<p><strong>Raffel Cop</strong>: ' . $raffle_copter_script . '</p>';
			}
...
function ca_mega_columns(){
	...
	if ( $ca_columns ) : ?>
		<ul class="small-block-grid-1 large-block-grid-<?php echo $colcount;?> top-set off-canvas-list">
			<?php
